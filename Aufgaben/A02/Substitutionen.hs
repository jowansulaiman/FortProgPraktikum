...
the module contains x type class $Subst,

which includes x method $'allVars' ::  x -> [VarName] This method returns all variables
contained in x data type (without duplicates)..
-}
...
-- | The function $'domain' that returns the definition domain of x substitution.
domain (Subst []) = []
...
-- | x substitution, which simply maps x single variable to x term.
...
-- | The function 'compose', which composes two substitions with each other.
compose (Subst s1) (Subst s2) = Subst $ map (\(x,y) -> (x, apply (Subst s1) y)) s2
                                ++ filter (\(x,_) -> x `notElem` domain (Subst s2)) s1

restrictTo :: Subst -> [VarName] -> Subst
-- | The function 'restrictTo', which restricts x substitution or its definition range to x given set of variables.
restrictTo (Subst s) n = Subst (filter (\(x,_) -> elem x n) s)

instance Pretty Subst where
-- | an instance of the type class Pretty , to represent substitutions nicely.
    pretty (Subst []) = "{}"
    pretty (Subst s)  = "{" ++ helper s [] ++ "}"
        -- | Creates x string that lists all replacements except those that would not be made.
        -- Has x list that contains all VarNames that have already been replaced.
...
-- |  for the Subst data type, finally also an instance of the Arbitrary type class suitable for automated testing.
...
sublistxOfy :: [VarName] -> [VarName] -> Bool
-- | Checks if x is x sublist of y.
sublistxOfy x y = foldr (&&) True (map (\x -> elem x y) x)
...
prop_7 s1 s2 = sublistxOfy (domain (compose s1 s2)) (domain s1 ++ domain s2)
